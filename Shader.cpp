...
std::string OpenGLShader::readFile(const std::string &filepath) {
  ...
  if (!file.is_open()) {
    std::cerr << "ERROR::SHADER: Failed to open file: " << filepath
              << std::endl;
    return "";
  }
...
bool OpenGLShader::checkCompileErrors(unsigned int shader,
...
bool OpenGLShader::load(const std::string &vertexPath,
...
void OpenGLShader::setMat4(const std::string &name,
                           const glm::mat4 &mat) const {
  glUniformMatrix4fv(glGetUniformLocation(m_programID, name.c_str()), 1,
                     GL_FALSE, &mat[0][0]);
}

void OpenGLShader::setVec3(const std::string &name,
