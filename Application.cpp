...
Application::Application(std::unique_ptr<AppConfig> config,
                         std::unique_ptr<IWindow> window,
                         std::unique_ptr<IRenderer> renderer,
                         std::unique_ptr<IShader> shader,
                         std::unique_ptr<IMeshGenerator> meshGenerator)
    : config(std::move(config)), window(std::move(window)),
      renderer(std::move(renderer)), shader(std::move(shader)),
      meshGenerator(std::move(meshGenerator)) {}

bool Application::init() {
  this->terrainMesh = std::make_unique<Mesh>(this->meshGenerator->generate(
      this->config->heightmapPath, this->config->terrainMaxHeight,
      this->config->terrainScaleXZ));
  if (this->terrainMesh->vertices.empty()) {
    std::cerr << "Failed to generate terrain mesh" << std::endl;
    return false;
  }

  if (!this->window->init(this->config->windowWidth, this->config->windowHeight,
...
void Application::setupCamera() {
  ...
  this->position = glm::vec3(
      terrainW / 2.0f, this->config->terrainMaxHeight * 3.0f, -terrainD / 3.0f);
  glm::vec3 targetPos = glm::vec3(terrainW / 2.0f, 0.0f, terrainD / 2.0f);

  this->model = glm::mat4(1.0f);
  this->view =
      glm::lookAt(this->position, targetPos, glm::vec3(0.0f, 1.0f, 0.0f));
