...
TEST(HeightmapGeneratorTest, GeneratesCorrectlyFrom3x3) {
  ...
  ASSERT_FALSE(mesh.vertices.empty())
      << "Test failed. Did you create 'test_3x3.png' in the project root?";
  ...
  // Our test image is all black (0) except the center (1,1) pixel, which is
  // white (255). The vertex at (x=1, z=1) should be at height 10.0. Its index
  // in the 1D vertex array is (z * width + x) = (1 * 3 + 1) = 4.

  // 1. Check center vertex (index 4)
  const Vertex &centerVertex = mesh.vertices[4];
  EXPECT_FLOAT_EQ(centerVertex.position.x, 1.0f);
  EXPECT_FLOAT_EQ(centerVertex.position.y,
                  10.0f); // normHeight (1.0) * maxHeight (10.0)
  ...
  EXPECT_FLOAT_EQ(cornerVertex.position.y,
                  0.0f); // normHeight (0.0) * maxHeight (10.0)
  EXPECT_FLOAT_EQ(cornerVertex.position.z, 0.0f);

  // 3. Check center vertex normal
  // hL=0, hR=0, hD=0, hU=0. All neighbors are 0 height.
  // Normal = normalize(vec3(0 - 0, 2.0 * 1.0, 0 - 0)) = normalize(0, 2, 0) =
  // (0, 1, 0)
