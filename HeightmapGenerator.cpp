...
float HeightmapGenerator::getHeight(int x, int z, int width, int height,
...
Mesh HeightmapGenerator::generate(const std::string &filepath, float maxHeight,
                                  float scaleXZ) {
  Mesh mesh;
  ...
  if (width <= 1 || height <= 1) {
    std::cerr << "Error [HeightmapGenerator]: Heightmap is too small."
              << std::endl;
    stbi_image_free(data);
    return mesh;
  }

  std::cout << "Info [HeightmapGenerator]: Loaded " << width << " x " << height
            << " image." << std::endl;
  ...
  for (int z = 0; z < height; z++) {
    for (int x = 0; x < width; x++) {
      ...
      v.position = glm::vec3((float)x * scaleXZ, normalHeight * maxHeight,
                             (float)z * scaleXZ);
